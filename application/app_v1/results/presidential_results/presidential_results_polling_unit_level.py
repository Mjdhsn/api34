from app.application.app_v1.database import get_db,get_db
from app.application.app_v1.results.presidential_results.partytable import presidential_table_pu
...
def get__polling_ward_all_results(country_name="undefined",state_name="undefined", lga_name="undefined", ward_name="undefined"):
    with get_db() as conn:
        ...
        ward_result = f"""{presidential_table_pu['query']} select ward_name,  NNPP, APC,  PDP,   LP, AA,  AAC, ADC,  A,  ADP,   APGA,  APM,  APP,  BP,  NRM,  PRP,  SDP,  YPP,  ZLP, 
   		 Total_Registered_voters,Total_Accredited_voters,total_valid_votes,Total_Rejected_votes,total_vote_casted,remarks from wt where 1=1 {state_query} {lga_query} {ward_query} """
 ...
 Total_Registered_voters,Total_Accredited_voters,total_valid_votes,Total_Rejected_votes,total_vote_casted,remarks from pu  where 1=1 {state_query} {lga_query} {ward_query} order by pu_id"""
        key_values = []
        execute_queries = []
...
def get_polling_state_all_results(country_name="undefined",state_name="undefined"):
    with get_db() as conn:
        ...
        state_result = f"""{presidential_table_pu['query']} select state_name,  NNPP, APC,  PDP,   LP, AA,  AAC, ADC,  A,  ADP,   APGA,  APM,  APP,  BP,  NRM,  PRP,  SDP,  YPP,  ZLP, 
...
def get_polling_country_all_results(country_name):
    with get_db() as conn:
        cur = conn.cursor()
        ...
        country_result = f"""{presidential_table_pu['query']} select country_name,  NNPP, APC,  PDP,   LP, AA,  AAC, ADC,  A,  ADP,   APGA,  APM,  APP,  BP,  NRM,  PRP,  SDP,  YPP,  ZLP, 
