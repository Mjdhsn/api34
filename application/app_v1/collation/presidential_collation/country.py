...
def getCountryResult(COUNTRY_NAME):
    with get_db2() as conn:
        ...
        try:
            ...
            results1 = results1.to_json(orient="records")
...
def cancelCountryResult(COUNTRY_NAME, data={}):
    ...
    with get_db2() as conn:
        ...
        sql = f"""Update country_result_table SET status='canceled', A=0, AA=0, AAC=0, ADC=0, ADP=0, APC=0, APGA=0, APM=0, APP=0, BP=0, LP=0, NNPP=0, NRM=0, PDP=0, PRP=0, SDP=0, TOTAL_VALID_VOTES_C=0, Total_Rejected_votes=0,TOTAL_VOTES_CAST_C=0, YPP=0, ZLP=0 , date_time ='{timer}' WHERE COUNTRY_ID={COUNTRY_NAME}"""
