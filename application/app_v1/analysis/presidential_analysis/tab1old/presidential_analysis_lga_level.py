...
values_win = {
"win_l":f"""{presidential_table['query']} , win_l as 
...
""",
"win_s":f"""{presidential_table['query']} ,win_s as
...
# QUERIES
conditions_lga = {
    "total": f"""{presidential_table['query']} SELECT COUNT(*) as  count1 FROM lgat""",
    "total_registered_votes_table": f"""{presidential_table['query']} select  lga_name, Total_Registered_voters,Total_Accredited_voters,Total_Rejected_votes,remarks FROM lgat""",
    "total_registered_votes": f"""{presidential_table['query']} SELECT COALESCE(sum(Total_Registered_voters),0) as  count1 FROM lgat""",
    "canceled": f"""{presidential_table['query']} SELECT count(*) as  count1 FROM lgat where status ="canceled" """,  
    "canceled_table": f"""{presidential_table['query']} SELECT lga_name, Total_Registered_voters,Total_Accredited_voters,Total_Rejected_votes,remarks FROM lgat where status ="canceled" """,
    "total_registered_canceled_voters": f"""{presidential_table['query']} select COALESCE(sum(Total_Registered_voters),0) as  count1 FROM lgat where status ='canceled' """ ,   
    "collated": f"""{presidential_table['query']}   SELECT sum(case when status = "collated" OR status = "canceled" then 1 else 0 end) as  count1 FROM lgat""",
    "collated_table": f"""{presidential_table['query']} SELECT  lga_name, Total_Registered_voters,Total_Accredited_voters,Total_Rejected_votes,remarks  FROM lgat WHERE  (status = 'collated' OR status='canceled')""",
    "total_registered_collated_voters": f"""{presidential_table['query']} select COALESCE(sum(Total_Registered_voters),0) as  count1 FROM lgat where (status = 'collated' OR status='canceled')""",
    "un_collated": f"""{presidential_table['query']} SELECT COUNT(*) as  count1   FROM lgat where status='non collated'""",
    "un_collated_table":f"""{presidential_table['query']} SELECT  lga_name, Total_Registered_voters,Total_Accredited_voters,Total_Rejected_votes,remarks FROM lgat where status='non collated'""",
    "total_registered_uncollated_voters": f"""{presidential_table['query']} select COALESCE(sum(Total_Registered_voters),0) as  count1 FROM lgat where status='non collated'""",
    ...
    "over_voting": f"""{presidential_table['query']} SELECT count(*) as count1 FROM lgat WHERE over_vote_values>0""",
    "over_voting_table":f"""{presidential_table['query']} SELECT lga_name,over_vote_values,remarks,percentage_voters_turnout  FROM lgat WHERE over_vote_values>0""",
    "total_over_voting": f"""{presidential_table['query']} select sum(over_vote_values) as over_votes_figuers FROM lgat WHERE over_vote_values>0""",
...
conditions_state = {
    "total": f"""{presidential_table['query']} SELECT COUNT(*) as  count1 FROM lgat""",
    "total_registered_votes_table": f"""{presidential_table['query']} select state_name, lga_name, Total_Registered_voters,Total_Accredited_voters,Total_Rejected_votes,remarks FROM lgat""",
    "total_registered_votes": f"""{presidential_table['query']} SELECT COALESCE(sum(Total_Registered_voters),0) as  count1 FROM lgat""",
    "canceled": f"""{presidential_table['query']} SELECT count(*) as  count1 FROM lgat where status ="canceled" """,  
    "canceled_table": f"""{presidential_table['query']} SELECT state_name,lga_name, Total_Registered_voters,Total_Accredited_voters,Total_Rejected_votes,remarks FROM lgat where status ="canceled" """,
    "total_registered_canceled_voters": f"""{presidential_table['query']} select COALESCE(sum(Total_Registered_voters),0) as  count1 FROM lgat where status ='canceled' """ ,   
    "collated": f"""{presidential_table['query']}   SELECT sum(case when status = "collated" OR status = "canceled" then 1 else 0 end) as  count1 FROM lgat""",
    "collated_table": f"""{presidential_table['query']} SELECT  state_name, lga_name, Total_Registered_voters,Total_Accredited_voters,Total_Rejected_votes,remarks  FROM lgat WHERE  (status = 'collated' OR status='canceled')""",
    "total_registered_collated_voters": f"""{presidential_table['query']} select COALESCE(sum(Total_Registered_voters),0) as  count1 FROM lgat where (status = 'collated' OR status='canceled')""",
    "un_collated": f"""{presidential_table['query']} SELECT COUNT(*) as  count1   FROM lgat where status='non collated'""",
    "un_collated_table":f"""{presidential_table['query']} SELECT  state_name,lga_name, Total_Registered_voters,Total_Accredited_voters,Total_Rejected_votes,remarks FROM lgat where status='non collated'""",
    "total_registered_uncollated_voters": f"""{presidential_table['query']} select COALESCE(sum(Total_Registered_voters),0) as  count1 FROM lgat where status='non collated'""",
    ...
    "over_voting": f"""{presidential_table['query']} SELECT count(*) as count1 FROM lgat WHERE over_vote_values>0""",
    "over_voting_table":f"""{presidential_table['query']} SELECT state_name,lga_name, over_vote_values,remarks,percentage_voters_turnout  FROM lgat WHERE over_vote_values>0""",
    "total_over_voting": f"""{presidential_table['query']} select sum(over_vote_values) as over_votes_figuers FROM lgat WHERE over_vote_values>0""",
