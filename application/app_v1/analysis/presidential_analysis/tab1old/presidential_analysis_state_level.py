...
# QUERIES
conditions_state = {
    "total": f"""{presidential_table['query']} SELECT COUNT(*) as  count1 FROM  st""",
    "total_registered_votes_table": f"""{presidential_table['query']} select  state_name, Total_Registered_voters,Total_Accredited_voters,Total_Rejected_votes,remarks FROM  st""",
    "total_registered_votes": f"""{presidential_table['query']} SELECT COALESCE(sum(Total_Registered_voters),0) as  count1 FROM  st""",
    "canceled": f"""{presidential_table['query']} SELECT count(*) as  count1 FROM  st where status ="canceled" """,  
    "canceled_table": f"""{presidential_table['query']} SELECT state_name, Total_Registered_voters,Total_Accredited_voters,Total_Rejected_votes,remarks FROM  st where status ="canceled" """,
    "total_registered_canceled_voters": f"""{presidential_table['query']} select COALESCE(sum(Total_Registered_voters),0) as  count1 FROM  st where status ='canceled' """ ,   
    "collated": f"""{presidential_table['query']}   SELECT sum(case when status = "collated" OR status = "canceled" then 1 else 0 end) as  count1 FROM  st""",
    "collated_table": f"""{presidential_table['query']} SELECT  state_name, Total_Registered_voters,Total_Accredited_voters,Total_Rejected_votes,remarks  FROM  st WHERE  (status = 'collated' OR status='canceled')""",
    "total_registered_collated_voters": f"""{presidential_table['query']} select COALESCE(sum(Total_Registered_voters),0) as  count1 FROM  st where (status = 'collated' OR status='canceled')""",
    "un_collated": f"""{presidential_table['query']} SELECT COUNT(*) as  count1   FROM  st where status='non collated'""",
    "un_collated_table":f"""{presidential_table['query']} SELECT  state_name, Total_Registered_voters,Total_Accredited_voters,Total_Rejected_votes,remarks FROM  st where status='non collated'""",
    "total_registered_uncollated_voters": f"""{presidential_table['query']} select COALESCE(sum(Total_Registered_voters),0) as  count1 FROM  st where status='non collated'""",
    ...
    "over_voting": f"""{presidential_table['query']} SELECT count(*) as count1 FROM  st WHERE over_vote_values>0""",
    "over_voting_table":f"""{presidential_table['query']} SELECT state_name,over_vote_values,remarks,percentage_voters_turnout  FROM  st WHERE over_vote_values>0""",
    "total_over_voting": f"""{presidential_table['query']} select sum(over_vote_values) as over_votes_figuers FROM  st WHERE over_vote_values>0""",
