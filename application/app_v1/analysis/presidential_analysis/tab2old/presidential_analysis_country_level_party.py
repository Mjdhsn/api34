...
# QUERIES
conditions_country = {
    "collation_status": f"""{values['ct']} select case when status='collated' then 'collated'  when status='non collated' then 'non coalated' else 'canceled' end as 'coallation status' from country_result_table""",
    "over_voting_status": f"""{values['ct']} select (case when over_vote_values >0 then remarks else 'NO Over Voting!!' end) as  over_voting_status from ct""",
    "over_voting_figure": f"""{values['ct']} select  over_vote_values as Total_over_vote_figures from ct""",
   
    "total_registered_voters": f"""{values['ct']} SELECT Total_Registered_voters as count1  FROM ct""",
    "total_accredited_voters": f"""{values['ct']} SELECT Total_Accredited_voters as count1  from ct""",
    "total_rejected_votes": f"""{values['ct']} SELECT Total_Rejected_votes  as count1 from ct """,
    "total_valid_votes": f"""{values['ct']} SELECT total_valid_votes as count1  from ct """,
    "total_vote_casted": f"""{values['ct']} SELECT total_vote_casted as count1 from ct""",
    "percentage_voters_turnout": f"""{values['ct']} SELECT percentage_voters_turnout as count1  from ct""",
