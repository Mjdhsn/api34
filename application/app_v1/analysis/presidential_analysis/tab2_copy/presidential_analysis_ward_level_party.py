...
import pandas as pd
import json
from app.application.app_v1.analysis.presidential_analysis.tab2_copy.party_table import presidential_table_ward
    ...
    "ward": {
        ...
        iff(total_vote_casted>0, concat(round(votes/total_vote_casted*100,2),'%'),'Voting in progress...') as percentage_score FROM win_ward 
 		where 1=1 and state_id={state_name} and lga_id={lga_name} and ward_id={ward_name}"""
            ...
            for key1, val in conditions_ward_ward['ward'].items():
                if key1 == 'values':
            ...
            query2 = []
            complete2 = []
            queries = len(conditions_ward_ward['ward']['values'])
            queries2 = len(conditions_ward_ward['ward']['tables'])
            import time
