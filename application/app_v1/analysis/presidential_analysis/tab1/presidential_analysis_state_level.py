...
from app.application.app_v1.analysis.presidential_analysis.tab1.party_table import presidential_table_state
import json
...
conditions_state = {
    ...
    "total_registered_votes_table": f"""{presidential_table_state['query']} select  state_name, Total_Registered_voters,total_valid_votes,Total_Rejected_votes,total_vote_casted,remarks FROM  st""",
    ...
    "canceled_table": f"""{presidential_table_state['query']} SELECT state_name, Total_Registered_voters,total_valid_votes,Total_Rejected_votes,total_vote_casted,remarks FROM  st where status ='canceled' """,
    ...
    "collated_table": f"""{presidential_table_state['query']} SELECT  state_name, Total_Registered_voters,total_valid_votes,Total_Rejected_votes,total_vote_casted,remarks  FROM  st WHERE  (status = 'collated' OR status='canceled')""",
    ...
    "un_collated_table":f"""{presidential_table_state['query']} SELECT  state_name, Total_Registered_voters,total_valid_votes,Total_Rejected_votes,total_vote_casted,remarks FROM  st where status='non collated'""",
...
conditions_country = {
    "total": f"""{presidential_table_state['query']} SELECT COUNT(*) as  count1 FROM  st""",
    "total_registered_votes_table": f"""{presidential_table_state['query']} select  state_name, Total_Registered_voters,total_valid_votes,Total_Rejected_votes,total_vote_casted FROM  st""",
    "total_registered_votes": f"""{presidential_table_state['query']} SELECT COALESCE(sum(Total_Registered_voters),0) as  count1 FROM  st""",
    'canceled': f"""{presidential_table_state['query']} SELECT count(*) as  count1 FROM  st where status ='canceled' """,  
    "canceled_table": f"""{presidential_table_state['query']} SELECT state_name, Total_Registered_voters,total_valid_votes,Total_Rejected_votes,total_vote_casted FROM  st where status ='canceled' """,
    "total_registered_canceled_voters": f"""{presidential_table_state['query']} select COALESCE(sum(Total_Registered_voters),0) as  count1 FROM  st where status ='canceled' """ ,   
    'collated': f"""{presidential_table_state['query']}   SELECT sum(case when status = 'collated' OR status = 'canceled' then 1 else 0 end) as  count1 FROM  st""",
    "collated_table": f"""{presidential_table_state['query']} SELECT  state_name, Total_Registered_voters,total_valid_votes,Total_Rejected_votes,total_vote_casted  FROM  st WHERE  (status = 'collated' OR status='canceled')""",
    "total_registered_collated_voters": f"""{presidential_table_state['query']} select COALESCE(sum(Total_Registered_voters),0) as  count1 FROM  st where (status = 'collated' OR status='canceled')""",
    "un_collated": f"""{presidential_table_state['query']} SELECT COUNT(*) as  count1   FROM  st where status='non collated'""",
    "un_collated_table":f"""{presidential_table_state['query']} SELECT  state_name, Total_Registered_voters,total_valid_votes,Total_Rejected_votes,total_vote_casted FROM  st where status='non collated'""",
    "total_registered_uncollated_voters": f"""{presidential_table_state['query']} select COALESCE(sum(Total_Registered_voters),0) as  count1 FROM  st where status='non collated'""",
