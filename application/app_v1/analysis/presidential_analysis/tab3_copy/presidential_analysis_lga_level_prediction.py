...
from app.application.app_v1.analysis.presidential_analysis.tab3_copy.party_table import presidential_table_lga
...
# QUERIES
conditions_lga = {
       "total_registered_voters": f"""{presidential_table_lga['query']} SELECT  COALESCE(sum(Total_Registered_voters),0) as count1 from lgat""",
       "total_accredited_voters": f"""{presidential_table_lga['query']}  SELECT  COALESCE(sum(Total_Accredited_voters),0) as count1 from lgat""",
       "total_rejected_votes": f"""{presidential_table_lga['query']}  SELECT  COALESCE(sum(Total_Rejected_votes),0) as count1 from lgat """,
       "total_valid_votes": f"""{presidential_table_lga['query']}  SELECT  COALESCE(sum(total_valid_votes),0) as count1  from lgat """,
        "total_vote_casted": f"""{presidential_table_lga['query']}  SELECT  COALESCE(sum(total_vote_casted),0) as count1 from lgat""",
        "percentage_voters_turnout": f"""{presidential_table_lga['query']}  SELECT percentage_voters_turnout as count1  from lgat """,
       "over_voting": f"""{presidential_table_lga['query']}  SELECT count(*) as count1  from lgat where over_vote_values>0""",
       "total_over_voting": f"""{presidential_table_lga['query']}  select COALESCE(sum(over_vote_values),0) as count1 from lgat where over_vote_values>0""",
        # "total_over_voting_table":f"""{presidential_table_lga['query']}  SELECT state_name,lga_name,over_vote_values,remarks,percentage_voters_turnout,Total_Registered_voters  from lgat where over_vote_values>0""",
       "message": f"""{presidential_table_lga['query']} 

SELECT CASE 			
 	WHEN ((select count(*)  FROM lgat where (status='non collated')) = (select count(*)  FROM lgat))		
...
END 
	 ...
	 """,
       ...
       "party_table": f"""{presidential_table_lga['query']}
