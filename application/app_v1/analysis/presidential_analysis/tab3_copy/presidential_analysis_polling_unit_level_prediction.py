...
from app.application.app_v1.analysis.presidential_analysis.tab3_copy.party_table import presidential_table_pu
...
conditions_pu = {
       "total_registered_voters": f"""{presidential_table_pu['query']} SELECT  COALESCE(sum(Total_Registered_voters),0) as count1 FROM pu""",
        ...
        "total_valid_votes": f"""{presidential_table_pu['query']}  SELECT  COALESCE(sum(total_valid_votes),0) as count1  from pu """,
         ...
         "percentage_voters_turnout": f"""{presidential_table_pu['query']}  SELECT percentage_voters_turnout as count1 from pu""",
