...
conditions_state = {
    ...
    "total_registered_votes_table": f"""{presidential_table_state['query']} select  district_name, Total_Registered_voters,total_valid_votes,Total_Rejected_votes,total_vote_casted,remarks FROM  st""",
    ...
    "canceled_table": f"""{presidential_table_state['query']} SELECT district_name, Total_Registered_voters,total_valid_votes,Total_Rejected_votes,total_vote_casted,remarks FROM  st where status ='canceled' """,
    ...
    "collated_table": f"""{presidential_table_state['query']} SELECT  district_name, Total_Registered_voters,total_valid_votes,Total_Rejected_votes,total_vote_casted,remarks  FROM  st WHERE  (status = 'collated' OR status='canceled')""",
    ...
    "un_collated_table":f"""{presidential_table_state['query']} SELECT  district_name, Total_Registered_voters,total_valid_votes,Total_Rejected_votes,total_vote_casted,remarks FROM  st where status='non collated'""",
    ...
    "over_voting_table":f"""{presidential_table_state['query']} SELECT district_name,over_vote_values,remarks,percentage_voters_turnout  FROM  st WHERE over_vote_values>0""",
    ...
    "party_graph":f"""{presidential_table_state['query']} SELECT ROW_NUMBER() OVER(PARTITION BY district_name ORDER BY votes DESC) AS row_num,party,votes,	
...
# state results
def get_state_state_all_results(state_name="undefined",district_name="undefined"):
    with get_db2() as conn:
        ...
        if state_name and state_name != "undefined":
            country_query = f" AND state_id ={state_name}"
        if district_name and district_name != "undefined":
            state_query = f" AND district_id={district_name}"
...
def get_state_country_all_results(state_name="undefined"):
    with get_db2() as conn:
        cur = conn.cursor()
        country_query = ""
