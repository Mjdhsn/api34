...
from app.application.app_v1.analysis.senate_analysis.tab1.party_table import presidential_table_state
import json
...
conditions_country = {
 ...
 else 'check manually' end AS count1 from sen_district_table""",
    "over_voting_status": f"""{presidential_table_state['query']}select (case when over_vote_values >0 then remarks else 'NO Over Voting!!' end) as  count1 from dist""",
    "over_voting_figure": f"""{presidential_table_state['query']} select  over_vote_values as count1 from dist""",
   
    "total_registered_voters": f"""{presidential_table_state['query']} SELECT Total_Registered_voters as count1  from dist""",
    # "total_accredited_voters": f"""{presidential_table_state['query']} SELECT Total_Accredited_voters as count1  from dist""",
    "total_rejected_votes": f"""{presidential_table_state['query']} SELECT Total_Rejected_votes  as count1 from dist """,
    "total_valid_votes": f"""{presidential_table_state['query']} SELECT total_valid_votes as count1  from dist """,
    "total_vote_casted": f"""{presidential_table_state['query']} SELECT total_vote_casted as count1 from dist""",
    "percentage_voters_turnout": f"""{presidential_table_state['query']} SELECT percentage_voters_turnout as count1  from dist""",
...
def get_country_country_all_results(state_name="undefined",district_name="undefined"):
