...
# QUERIES
conditions_state = {
       "total_registered_voters": f"""{presidential_table_state['query']} SELECT  COALESCE(sum(Total_Registered_voters),0) as count1 from st""",
       "total_accredited_voters": f"""{presidential_table_state['query']}  SELECT  COALESCE(sum(Total_Accredited_voters),0) as count1 from st""",
       "total_rejected_votes": f"""{presidential_table_state['query']}  SELECT  COALESCE(sum(Total_Rejected_votes),0) as count1 from st """,
       "total_valid_votes": f"""{presidential_table_state['query']}  SELECT  COALESCE(sum(total_valid_votes),0) as count1  from st """,
        "total_vote_casted": f"""{presidential_table_state['query']}  SELECT  COALESCE(sum(total_vote_casted),0) as count1 from st""",
        "percentage_voters_turnout": f"""{presidential_table_state['query']}  SELECT percentage_voters_turnout as count1  from st""",
       "over_voting": f"""{presidential_table_state['query']}  SELECT count(*) as count1  from st where over_vote_values>0""",
       "total_over_voting": f"""{presidential_table_state['query']}  select COALESCE(sum(over_vote_values),0) as count1 from st where over_vote_values>0""",
        # "total_over_voting_table":f"""{presidential_table_state['query']}  SELECT state_name,over_vote_values,remarks,percentage_voters_turnout,Total_Registered_voters  from st where over_vote_values>0""",
       "message": f"""{presidential_table_state['query']} 
...
END
	 ...
	 """,
       ...
       "party_table": f"""{presidential_table_state['query']}
