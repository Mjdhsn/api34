...
import asyncio
import json
from app.application.app_v1.analysis.rep_analysis.tab1.party_table import presidential_table_pu
import json
import time
    ...
    "total": f"""{presidential_table_pu['query']} SELECT COUNT(*) as  count1 FROM pu""",
     ...
     "total_over_voting": f"""{presidential_table_pu['query']} select COALESCE(sum(over_vote_values),0) as over_votes_figuers from pu WHERE over_vote_values>0""",
      "party_graph":f"""{presidential_table_pu['query']} SELECT ROW_NUMBER() OVER(PARTITION BY pu_name ORDER BY votes DESC) AS row_num,party,votes,	
          IFF(total_vote_casted>0, concat(round(votes/total_vote_casted*100,2),'%'), 'Voting in progress... or Canceled')  as percentage_votes_casted FROM win_pu """
   }
...
conditions_districts = {
    "total": f"""{presidential_table_pu['query']} SELECT COUNT(*) as  count1 FROM pu""",
    "total_registered_votes_table": f"""{presidential_table_pu['query']} select  constituency_name,lga_name,ward_name,pu_name,pu_code, Total_Registered_voters,Total_Accredited_voters,Total_Rejected_votes,remarks from pu""",
    "total_registered_votes": f"""{presidential_table_pu['query']} SELECT COALESCE(sum(Total_Registered_voters),0) as  count1 from pu""",
    'canceled': f"""{presidential_table_pu['query']} SELECT count(*) as  count1 from pu where status ='canceled' """,  
    "canceled_table": f"""{presidential_table_pu['query']} SELECT constituency_name,lga_name,ward_name,pu_name,pu_code, Total_Registered_voters,Total_Accredited_voters,Total_Rejected_votes,remarks from pu where status ='canceled' """,
    "total_registered_canceled_voters": f"""{presidential_table_pu['query']} select COALESCE(sum(Total_Registered_voters),0) as  count1 from pu where status ='canceled' """ ,   
    'collated': f"""{presidential_table_pu['query']}   SELECT COALESCE(sum(case when status = 'collated' OR status = 'canceled' then 1 else 0 end),0)  as  count1 from pu""",
    "collated_table": f"""{presidential_table_pu['query']} SELECT  constituency_name,lga_name,ward_name,pu_name,pu_code, Total_Registered_voters,Total_Accredited_voters,Total_Rejected_votes,remarks  from pu WHERE  (status = 'collated' OR status='canceled')""",
    "total_registered_collated_voters": f"""{presidential_table_pu['query']} select COALESCE(sum(Total_Registered_voters),0) as  count1 from pu where (status = 'collated' OR status='canceled')""",
    "un_collated": f"""{presidential_table_pu['query']} SELECT COUNT(*) as  count1   from pu where status='non collated'""",
    "un_collated_table":f"""{presidential_table_pu['query']} SELECT  constituency_name,lga_name,ward_name,pu_name,pu_code, Total_Registered_voters,Total_Accredited_voters,Total_Rejected_votes,remarks from pu where status='non collated'""",
    "total_registered_uncollated_voters": f"""{presidential_table_pu['query']} select COALESCE(sum(Total_Registered_voters),0) as  count1 from pu where status='non collated'""",
    "total_registered_voters": f"""{presidential_table_pu['query']} SELECT Total_Registered_voters  from rept""",
    "total_accredited_voters": f"""{presidential_table_pu['query']} SELECT Total_Accredited_voters  from rept""",
    "total_rejected_votes": f"""{presidential_table_pu['query']} SELECT Total_Rejected_votes   from rept """,
    "total_valid_votes": f"""{presidential_table_pu['query']} SELECT total_valid_votes  from rept """,
    "total_vote_casted": f"""{presidential_table_pu['query']} SELECT total_vote_casted  from rept""",
    "percentage_voters_turnout": f"""{presidential_table_pu['query']} SELECT percentage_voters_turnout  from rept""",
    "over_voting": f"""{presidential_table_pu['query']} SELECT count(*) as count1 from pu WHERE over_vote_values>0""",
    "over_voting_table":f"""{presidential_table_pu['query']} SELECT constituency_name,lga_name,ward_name,pu_name,pu_code,over_vote_values,remarks,percentage_voters_turnout  from pu WHERE over_vote_values>0""",
    "total_over_voting": f"""{presidential_table_pu['query']} select COALESCE(sum(over_vote_values),0) as over_votes_figuers from pu WHERE over_vote_values>0""",
    "party_graph":f"""{presidential_table_pu['query']} SELECT ROW_NUMBER() OVER(PARTITION BY constituency_name ORDER BY votes DESC) AS row_num,party,votes as Scores,		-- 22
       IFF(total_vote_casted>0, concat(round(votes/total_vote_casted*100,2),'%'),
		'Collation has not started... or Canceled') as percentage_score FROM win_const """
}
...
def get__polling_pu_all_results(state_name="undefined",constituency_name="undefined", lga_name="undefined", ward_name="undefined",pu_name="undefined"):
    with get_db2() as conn:
        ...
        key_values = []
        execute_queries = []
        if state_name or constituency_name or lga_name or ward_name or pu_name:
...
def get__polling_ward_all_results(state_name="undefined",constituency_name="undefined", lga_name="undefined",ward_name="undefined"):
    with get_db2() as conn:
        cur = conn.cursor()
...
def get_polling_lga_all_results(state_name="undefined",constituency_name="undefined",lga_name="undefined"):
    with get_db2() as conn:
        cur = conn.cursor()
        state_query = ""
        district_query = ""
...
# state results
def get_polling_state_all_results(state_name="undefined",constituency_name="undefined"):
    with get_db2() as conn:
        cur = conn.cursor()
        state_query = ""
        district_query = ""
